...
import com.example.hp.echo.fragments.SongPlayingFragment.Statified.favoriteContent
import com.example.hp.echo.fragments.SongPlayingFragment.Statified.mediaplayer
import com.example.hp.echo.fragments.SongPlayingFragment.Statified.myActivity
...
class SongPlayingFragment : Fragment() {
    ...
    object Staticated {
        ...
        fun onSongComplete() {
            ...
            if (favoriteContent?.checkifIdExists(Statified.currentSongHelper?.songId?.toInt() as Int)
                            as Boolean) {
        ...
        fun playNext(check: String) {
            ...
            mediaplayer?.reset()
            try {
                mediaplayer?.setDataSource(myActivity, Uri.parse(Statified.currentSongHelper?.songPath))
                mediaplayer?.prepare()
                mediaplayer?.start()
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        var songId: Long? = 0
        try {
            path = arguments?.get(path.toString()) as Text?
            ...
            Statified.currentSongHelper?.songPath = path?.toString()
        ...
        if (fromFavBottomBar != null) {
            ...
            try {
                Statified.mediaplayer?.setDataSource(Statified.myActivity, Uri.parse(path.toString()))
                Statified.mediaplayer?.prepare()
        ...
        clickHandler()
  //      var visualizationHandler = Statified.DbmHandler.Factory.newVisualizerHandler(myActivity as Context, 0)
